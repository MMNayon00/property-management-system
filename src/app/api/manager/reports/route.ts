// Manager API: Get report data for assigned buildings
...
export async function GET(_req: NextRequest) {
  try {
    ...
    // Get current month rent records
    ...
    const [
      ...
      // Unpaid rent (current month rent records that are unpaid/partial)
      ...
      // Total rent records for current month
    ...
    const paidRecords = await prisma.rentRecord.count({
      where: {
        buildingId: { in: buildingIds },
        month: currentMonth,
        paymentStatus: "PAID",
    ...
    const collectionRate = totalRentRecords > 0 ? Math.round((paidRecords / totalRentRecords) * 100) : 0;
